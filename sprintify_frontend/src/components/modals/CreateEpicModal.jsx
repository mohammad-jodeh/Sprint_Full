...
export default function CreateEpicModal({ onClose, onCreate }) {
  const { projectId } = useParams();  const [form, setForm] = useState({
    name: "", // Keep as 'name' for form field, but send as 'title' to API
    description: "",
    assigneeId: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await createEpic(projectId, payload);
      toast.success("Epic created successfully!");
      onCreate(result);
      onClose();
    } catch (error) {
      console.error("Failed to create epic:", error);
      const errorMessage = error.response?.data?.message || error.message || "Failed to create epic";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Add a new epic to organize your issues
            </p>
          </div>
        </div>
        <button
